...
save(mod_oppg6, file = "reg_data_2010.rdata")
```
...
```{r}
save(mod_oppg6_mean, file = "reg_data_2024.rdata")
```
...
x_mean <- master %>%
  transmute(
    ...
    # lag log per kommune-år først (som du gjorde)
  ...
  summarise(
    ...
    # Gjennomsnitt av log boligpris over perioden
  ...
  # fjern alt som er NA/NaN/Inf i X + krev landsdel
  filter(
    is.finite(Ledighet_pct),
    is.finite(Hoy_utdanning_pct),
    is.finite(Arbeidsplasser_1k),
...
```{r}
save(m_6B, file = "reg_data_Paneldatasett_log.rdata")
```
...
cs_vcov   <- vcovCL(mod_oppg6,      cluster = ~ kommune)  # cluster på kommune
mean_vcov <- vcovHC(mod_oppg6_mean, type = "HC1")
B_vcov    <- vcovHC(m_6B,           type = "HC1")

# pakk modeller + vcov i én ryddig liste
reg_results <- list(
  models = list(
...
## Lagre alt
save(reg_results, file = "reg_data_sammenligning.rdata")


rm(list = c("reg_results", "cs_vcov", "mean_vcov", "B_vcov"))
load("reg_data_Paneldatasett_log.rdata")

modelsummary(
  reg_results$models,
  vcov = reg_results$vcov,
  statistic = "({std.error})",
  stars = TRUE,
  gof_omit = ".*"
)
...
```{r}
cs_vcov   <- vcovCL(mod_oppg6,      cluster = ~ kommune)
mean_vcov <- vcovHC(mod_oppg6_mean, type = "HC1")
B_vcov    <- vcovHC(m_6B,           type = "HC1")
...
ms_gt <- modelsummary(
  ...
  vcov = list(cs_vcov, mean_vcov, B_vcov),
  statistic = "({std.error})",
  stars = TRUE,
...
save(ms_gt, file = "modelsummary_oppg6.rdata")
```
...
save(mod_oppg7_2010, file = "reg_data_oppg7_2010.rdata")
```
...
save(mod_oppg7_mean, file = "reg_data_oppg7_mean.rdata")
```
...
save(m_7B, file = "reg_data_m_7B.rdata")
```

Denne modellen analyserer hvilke faktorer som er assosiert med prosentvis sysselsettingsvekst i kommunene.
