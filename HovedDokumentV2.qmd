...
flytting_wide <- flytting |>
  left_join(beftot, by = c("knr24", "år")) |>
  mutate(nettoflytting1k = round((nettoflytting / beftot) * 1000, 2)) |>
  mutate(nettoflytting1k = if_else(is.infinite(nettoflytting1k), 0, nettoflytting1k)) |>
  select(knr24, år, Alder, nettoflytting1k) |>
  pivot_wider(
    names_from = Alder,
    names_prefix = "nettoflytting1k_",
    values_from = nettoflytting1k
  )

#Pendling (rate per 1000 arbeidsplasser)
pendl_rate <- pendl |>
  mutate(nettoInnPendl1k = round((nettoPendl / Arbeidssted) * 1000, 2)) |>
  mutate(nettoInnPendl1k = if_else(is.infinite(nettoInnPendl1k), 0, nettoInnPendl1k)) |>
  select(knr24, år, nettoInnPendl1k)
...
![Caption](Prosentvis_arbeidsledighet_for_2024.jpg)
# Sammenligning sentralitet over tid
...
```{r}
kor_år(master, "andel_SSB.x", "nettoflytting1k_18-29")|>
knitr::kable(
    caption = "Arbeidsledighet og netto flytting"
  )
```
...
knitr::kable(
    caption = "Gjennomsnittlig KVM pris og andel innbyggere mellom 20-29"
  )
```
...
```{r}
kor_år(master, "KvPris_Blokkleiligheter", "nettoInnPendl1k")|>
knitr::kable(
...
```{r}
kor_år(master, "KvPris_Blokkleiligheter", "nettoflytting1k_18-29")
```
...
```{r}
kor_år(master, "nettoflytting1k_18-29", "befAndel_F20-29")
```
...
```{r}
kor_år(master, "nettoflytting1k_18-29", "nettoInnPendl1k")
```
