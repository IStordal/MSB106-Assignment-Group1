...
```{r}
arlig_endring_lag <- arlig_endring |>
  arrange(knr24, år) |>
  group_by(knr24) |>
  mutate(
    pct_syssel_lag1 = lag(pct_syssel, 1),
    d_boligpris_lag1 = lag(d_boligpris, 1),
    d_andel_ledighet_lag1 = lag(d_andel_ledighet, 1)
  ) |>
  ungroup()
```
...
```{r}
kor_kommune_lag <- arlig_endring_lag |>
  group_by(knr24) |>
  summarise(
    kor_bef_syssel_lag1 =
      if (sum(complete.cases(d_bef, pct_syssel_lag1)) >= 2)
        cor(d_bef, pct_syssel_lag1, use = "complete.obs")
      else NA_real_,

    kor_bolig_flytt_lag1 =
      if (sum(complete.cases(d_nettoflytt1k, d_boligpris_lag1)) >= 2)
        cor(d_nettoflytt1k, d_boligpris_lag1, use = "complete.obs")
      else NA_real_,

    kor_ledig_syssel_lag1 =
      if (sum(complete.cases(pct_syssel, d_andel_ledighet_lag1)) >= 2)
        cor(pct_syssel, d_andel_ledighet_lag1, use = "complete.obs")
      else NA_real_,
...
summary(kor_kommune_lag$kor_bef_syssel_lag1)
