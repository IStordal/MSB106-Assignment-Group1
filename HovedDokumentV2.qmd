...
folke_growth <- master %>%
  ...
  mutate(
    folkevekst_pct = 100 * ( .data[[paste0("bef_", period_end)]] - .data[[paste0("bef_", period_start)]] ) /
      .data[[paste0("bef_", period_start)]]
  ) %>%
...
# Antall kommuner i regresjonsutvalget (gjennomsnitt)
regdata_mean %>%
  summarise(Antall_kommuner = n()) %>%
  knitr::kable()

# Regresjonsmodell (gjennomsnitt)
mod_oppg6_mean <- lm(
  folkevekst_pct ~ Ledighet_pct + Hoy_utdanning_pct + Arbeidsplasser_1k +
    Nettoflytting_1k + Nettopendling_1k + ln_Boligpris + Sentralitet + Landsdel,
  data = regdata_mean
...
Kommuner som i gjennomsnitt har vært mer sentrale i perioden har hatt høyere folketallsvekst.
Effekten er fortsatt relativt liten per enhet, men tydelig (én enhets høyere sentralitet er assosiert med rundt 0,09 prosentpoeng høyere folketallsvekst, alt annet likt).

Boligprisene har også i denne modellen en sterk positiv sammenheng med befolkningsutviklingen.
En økning i logaritmen til boligprisen er assosiert med rundt 19 prosentpoeng høyere folketallsvekst, noe som indikerer at boligpriser henger sammen med strukturelle forhold som påvirker bosettingsmønstren

Antall arbeidsplasser per 1000 innbyggere har en positiv og statistisk signifikant sammenheng med folketallsvekst.
En økning på 10 arbeidsplasser per 1000 innbyggere er forbundet med om lag 0,8 prosentpoeng høyere folketallsvekst.
Tilsvarende har netto innflytting også i denne modellen en tydelig positiv effekt: én ekstra person i netto innflytting per 1000 innbyggere er assosiert med rundt 0,55 prosentpoeng høyere folketallsvekst.

Netto utpendling har en negativ og statistisk signifikant sammenheng med folketallsutviklingen.
...
Arbeidsledighet har også en negativ sammenheng med folketallsvekst, men effekten er ikke statistisk signifikant i denne modellspesifikasjonen.

Dummyvariablene for landsdel er ikke statistisk signifikante.
Dette indikerer at når det kontrolleres for sentralitet, boligpriser, arbeidsmarked, flytting og pendling, har landsdel i seg selv liten `egen/selvstendig` betydning for folketallsutviklingen.

Modellen har høy forklaringsgrad, med en justert R² på rundt 0,74.
Dette innebærer at om lag 75 prosent av variasjonen i folketallsvekst mellom kommunene kan forklares av variablene i modellen.
Sammenlignet med startår-modellen er forklaringsgraden noe høyere, noe som tyder på at gjennomsnittsverdier over tid fanger opp mer av de langsiktige strukturelle forskjellene mellom kommunene.

Sammenlignet med modellen basert på startårverdier (år 2010) gir gjennomsnittsmodellen noe høyere forklaringsgrad, men hovedmønstrene er de samme: sentralitet, boligpriser, arbeidsmarked og flyttemønstre er sentrale faktorer for folketallsutviklingen i norske kommuner.
