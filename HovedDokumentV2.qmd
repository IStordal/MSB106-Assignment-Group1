...
```{r}
# 1) Sysselsetting i startår og sluttår + vekst (%)
df_7_syss <- df_6 %>%
  inner_join(df_6_period, by = "knr24") %>%
  group_by(knr24, start_år, end_år) %>%
  summarise(
...
# 2) Bygg "df_7_csB": X-gjennomsnitt + folketallsvekst + syssvekst
...
# 3) Oppg 7-modell (periodegjennomsnitt): sysselsettingsvekst forklart av folketallsvekst + X
m_7B <- lm(
