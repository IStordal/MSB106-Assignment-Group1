...
```{r}
kor_bef_syssel <- arlig_endring_lag |>
  group_by(knr24) |>
  summarise(
...
```{r}
kor_ledighet_syssel_lag <- arlig_endring_lag |>
  group_by(knr24) |>
  summarise(
...
```
Vi har kjørt en regresjon som viser sammenhengen mellom befolkning og sysselsetting for norske kommuner. Der vi byttet på befolkning og sysselsetting som avhengig variabel. I begge regresjonene er det en svært høy justert r-kvadrert. Det vil si at man ikke kan tolke resultatene kausalt. Man kan ikke bestemme hva som begrunnes av hva.

Videre forsøkte vi å sette kommune og år som fast effekt for å se om resultatene endret seg noe. Da ser vi at justert r-kvadrat er litt lavere, men fortsatt veldig høy. Det betyr også at vi ikke kan tolke resultatene kausalt.

Tidligere så vi på kurver for befolkningsutvikling og sysselsetting, der så vi at kurvene hadde en lik utvikling, noe som tyder på at de henger sammen. Det ser vi igjen her at forklaringsgraden er veldig høy. Men det forklarer ikke om sysselsetningen øker hvis befolkningen øker, eller omvendt. Man kan derfor ikke tolke det kausalt.
