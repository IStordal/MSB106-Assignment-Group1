...
panel_df7 <- master %>%
  transmute(
    ...
    # Folketall og sysselsetting (nivå)
    beftot = to_num(beftot.x),
    syss   = to_num(Arbeidssted.x),
  ...
  group_by(knr24) %>%
  mutate(
    # Årlig befolkningsvekst (log-differanse) -> X i oppg. 7
    d_ln_bef = log(beftot) - lag(log(beftot)),

    # Årlig sysselsettingsvekst (log-differanse) -> Y i oppg. 7
    d_ln_syss = log(pmax(syss, 1)) - lag(log(pmax(syss, 1)))
  ) %>%
  ungroup() %>%
  filter(
    is.finite(d_ln_syss),
...
# Two-way fixed effects: kommune + år
panel_fe7 <- plm(
  d_ln_syss ~ d_ln_bef + Ledighet_pct + Hoy_utdanning_pct +
