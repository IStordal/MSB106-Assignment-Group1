...
```{r}
#Har ikke fått gjort siste avsnitt i oppgave 6
#(valgfritt) sjekk at y faktisk varierer
panel_tidy %>%
  summarise(
    N = n(),
    Ant_kommuner = n_distinct(kommune),
    Ant_år = n_distinct(year),
    sd_y = sd(folkevekst_pct_aar, na.rm = TRUE),
    min_y = min(folkevekst_pct_aar, na.rm = TRUE),
    max_y = max(folkevekst_pct_aar, na.rm = TRUE)
  ) %>%
  knitr::kable()

# bygg pdata.frame
panel_reg <- panel_tidy %>%
  transmute(
    kommune, year,
    folkevekst_pct_aar,
...
panel_reg <- pdata.frame(panel_reg, index = c("kommune", "year"))

# TWFE (kommune + år)
mod_panel_twfe <- plm(
  folkevekst_pct_aar ~ Ledighet_pct + Hoy_utdanning_pct + Arbeidsplasser_1k +
    Nettoflytting_1k + Nettopendling_1k + ln_Boligpris,
  data = panel_reg,
  model = "within",
  effect = "twoways"
...
# cluster-robuste SE (kommune)
vc_twfe <- vcovHC(mod_panel_twfe, method = "arellano", type = "HC1", cluster = "group")

# "summary"-aktig output (robust)
twfe_ct <- lmtest::coeftest(mod_panel_twfe, vcov. = vc_twfe)
twfe_ct

# ryddig tabell (kable)
twfe_tab <- data.frame(
  term = rownames(twfe_ct),
  estimate = twfe_ct[, 1],
  std.error = twfe_ct[, 2],
  statistic = twfe_ct[, 3],
  p.value = twfe_ct[, 4],
...
twfe_tab %>%
  knitr::kable(digits = 3, caption = "Panel TWFE (plm) – cluster SE (kommune)")

#modell (t-1 forklaringsvariabler)
panel_reg_l1 <- panel_tidy %>%
  ...
  transmute(
    kommune, year,
    folkevekst_pct_aar,
    Ledighet_pct_l1, Hoy_utdanning_pct_l1, Arbeidsplasser_1k_l1,
    Nettoflytting_1k_l1, Nettopendling_1k_l1, ln_Boligpris_l1
  )

panel_reg_l1 <- pdata.frame(panel_reg_l1, index = c("kommune", "year"))

mod_panel_twfe_l1 <- plm(
  folkevekst_pct_aar ~ Ledighet_pct_l1 + Hoy_utdanning_pct_l1 + Arbeidsplasser_1k_l1 +
    Nettoflytting_1k_l1 + Nettopendling_1k_l1 + ln_Boligpris_l1,
  data = panel_reg_l1,
  model = "within",
  effect = "twoways"
...
vc_twfe_l1 <- vcovHC(mod_panel_twfe_l1, method = "arellano", type = "HC1", cluster = "group")

twfe_l1_ct <- lmtest::coeftest(mod_panel_twfe_l1, vcov. = vc_twfe_l1)
twfe_l1_ct

twfe_l1_tab <- data.frame(
  term = rownames(twfe_l1_ct),
  estimate = twfe_l1_ct[, 1],
  std.error = twfe_l1_ct[, 2],
  statistic = twfe_l1_ct[, 3],
  p.value = twfe_l1_ct[, 4],
  row.names = NULL
)

twfe_l1_tab %>%
  knitr::kable(digits = 3, caption = "Panel TWFE lag1 (plm) – cluster SE (kommune)")

# SAMMENLIGNING (robust, uten modelsummary)
# (Hvis du vil ha alt i én "modelsummary"-tabell: bruk fixest i stedet.
# Med plm lager vi derfor en enkel sammenligning ved å vise tre tabeller.)

# 1) lm (tverrsnitt) – vanlig summary (eller lag egen broom-tabell)
summary(mod_oppg6_mean)

broom::tidy(mod_oppg6_mean) %>%
  knitr::kable(digits = 3, caption = "Tverrsnitt (mean) – lm")

# 2) Panel TWFE-tabellen er 'twfe_tab' (over)
# 3) Panel TWFE lag1-tabellen er 'twfe_l1_tab' (over)
```
