...
![Caption](Bilde_universitetsutdannig.png)
### Prosentvis netto utflytting pr 1000 innbyggere
...
syssvekst0024 |>
  ...
  mutate(år = as.Date.character(år, format = "%Y")) |>
  ggplot(
  aes(x = år, y = Arbeidssted, group = klasse, color = klasse)) + geom_line(size = 0.8) + 
  theme(legend.position = "bottom") + scale_x_date(date_breaks = "5 years", date_labels = "%Y") + ylab("sysselsatte")
```
...
kvmpris0224 <- kvmBolig |>
      left_join(y = sentralitet24, join_by(knr24)) |> 
  group_by(klasse, år) |>
  summarise(KvPris_Eneboliger = sum(KvPris_Eneboliger), .groups = "drop")
...
kvmpris0224 |>
  ...
  mutate(år = as.Date.character(år, format = "%Y")) |>
  ggplot(
  aes(x = år, y = KvPris_Eneboliger, group = klasse, color = klasse)) + geom_line(size = 0.8) + 
...
utd8624 <- utdAndel |>
  group_by(klasse, år) |>
  summarise(UH_pct = sum(UH_pct), .groups = "drop")
```
...
flytt1024 <- flytting |>
  left_join(y = sentralitet24, join_by(knr24)) |> 
  group_by(klasse, år) |>
  summarise(nettoflytting = sum(nettoflytting), .groups = "drop")
```
...
flytt1024 |>
  ...
  mutate(år = as.Date.character(år, format = "%Y")) |>
  ggplot(
  aes(x = år, y = nettoflytting, group = klasse, color = klasse)) + geom_line(size = 0.8) + 
...
pendl0024 <- pendl |> 
  left_join(y = sentralitet24, join_by(knr24)) |> 
  group_by(klasse, år) |>
  summarise(nettoPendl = sum(nettoPendl), .groups = "drop")
```
...
pendl0024 |>
  ...
  mutate(år = as.Date.character(år, format = "%Y")) |>
  ggplot(
  aes(x = år, y = nettoPendl, group = klasse, color = klasse)) + geom_line(size = 0.8) + 
