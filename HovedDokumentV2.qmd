...
Sentralitetsindeks og befolkning

```{r}
kor_år(master, "index", "beftot.x")|>
knitr::kable(
    caption = "Sentralitetsindeks og befolkningsvekst"
  )
```
...
## Korelasjoner paneldata oppsumert

#Korelasjon mellom befolknningsutvikling og sysselsetting

```{r}
cat("Korelasjon mellom befolkningsutvikling og sysselsetting\n")
summary(kor_kommune$kor_bef_syssel)
```

#Korelasjon mellom arbeidsledighet og sysselsetting

```{r}
cat("Korelasjon mellom arbeidsledighet og sysselsetting\n")
summary(kor_kommune$kor_ledighet_syssel)
```

#Korelasjon mellom boligpriser og årlig innflytting

```{r}
cat("Korelasjon mellom boligpriser og årlig innflytting\n")
summary(kor_kommune$kor_bolig_flytt)
```

# Korelasjon mellom boligpriser og sysselsetting

```{r}
cat("Korelasjon mellom boligpriser og sysselsetting\n")
summary(kor_kommune$kor_syssel_bolig)
```

# Korelasjon mellom pendling og sysselsetting

```{r}
cat("Korelasjon mellom pendling og sysselsetting\n")
summary(kor_kommune$kor_pendl_syssel)
```

# Laggede endringer

```{r}
arlig_endring_lag1 <- arlig_endring |>
  mutate(år = år + 1)
```

```{r}
arlig_endring_med_lag <- arlig_endring |>
  left_join(
    arlig_endring_lag1,
    by = c("knr24", "år"),
    suffix = c("", "_lag1")
...
```{r}
kor_kommune_lag <- arlig_endring_med_lag |>
  group_by(knr24) |>
  summarise(

    # 1. Befolkningsendring (t-1) og sysselsetting (t)
    kor_bef_syssel_lag =
      if (sum(complete.cases(d_bef_lag1, pct_syssel)) >= 2)
        cor(d_bef_lag1, pct_syssel, use = "complete.obs")
      else NA_real_,

    # 2. Arbeidsledighet (t-1) og sysselsetting (t)
    kor_ledighet_syssel_lag =
      if (sum(complete.cases(d_andel_ledighet_lag1, pct_syssel)) >= 2)
        cor(d_andel_ledighet_lag1, pct_syssel, use = "complete.obs")
      else NA_real_,

    # 3. Boligpriser (t-1) og netto innflytting (t)
    kor_bolig_flytt_lag =
      if (sum(complete.cases(d_boligpris_lag1, d_nettoflytt1k)) >= 2)
        cor(d_boligpris_lag1, d_nettoflytt1k, use = "complete.obs")
      else NA_real_,

    # 4. Sysselsetting (t-1) og boligpriser (t)
    kor_syssel_bolig_lag =
      if (sum(complete.cases(pct_syssel_lag1, d_boligpris)) >= 2)
        cor(pct_syssel_lag1, d_boligpris, use = "complete.obs")
      else NA_real_,

    # 5. Innpendling (t-1) og sysselsetting (t)
    kor_pendl_syssel_lag =
      if (sum(complete.cases(d_innpendl1k_lag1, pct_syssel)) >= 2)
        cor(d_innpendl1k_lag1, pct_syssel, use = "complete.obs")
      else NA_real_,
...
```{r}
kor_sammenligning <- kor_kommune |>
  left_join(kor_kommune_lag, by = "knr24")
```

```{r}

cat("Befolkning og sysselsetting\n")
summary(kor_sammenligning$kor_bef_syssel)
cat("Befolkning og sysselsetting med lag\n")
summary(kor_sammenligning$kor_bef_syssel_lag)
```

```{r}
cat("Befolkning og flytting\n")
summary(kor_sammenligning$kor_bolig_flytt)
cat("Befolkning og flytting med lag\n")
summary(kor_sammenligning$kor_bolig_flytt_lag)
```

```{r}
arlig_endring_med_lag |>
  filter(knr24 == 0301) |>
  select(år, d_boligpris, d_boligpris_lag1) |>
  head(6)
```

# Spearsman vs Pearson

```{r}
kor_kommune_spearman <- arlig_endring |>
  group_by(knr24) |>
  summarise(
    
    # 1. Årlig endring i folketall og sysselsetting
    kor_bef_syssel =
      if (sum(complete.cases(d_bef, pct_syssel)) >= 2)
        cor(d_bef, pct_syssel, use = "complete.obs", method = "spearman")
      else NA_real_,
    
    # 2. Årlig endring i arbeidsledighet og sysselsetting
    kor_ledighet_syssel =
      if (sum(complete.cases(d_andel_ledighet, pct_syssel)) >= 2)
        cor(d_andel_ledighet, pct_syssel, use = "complete.obs", method = "spearman")
      else NA_real_,
    
    # 3. Årlig endring i boligpriser og netto flytting
    kor_bolig_flytt =
      if (sum(complete.cases(d_boligpris, d_nettoflytt1k)) >= 2)
        cor(d_boligpris, d_nettoflytt1k, use = "complete.obs", method = "spearman")
      else NA_real_,
    
    # 4. Årlig prosentvis endring i sysselsetting og boligpriser
    kor_syssel_bolig =
      if (sum(complete.cases(pct_syssel, d_boligpris)) >= 2)
        cor(pct_syssel, d_boligpris, use = "complete.obs", method = "spearman")
      else NA_real_,
    
    # 5. Årlig endring i innpendling og sysselsetting
    kor_pendl_syssel =
      if (sum(complete.cases(d_innpendl1k, pct_syssel)) >= 2)
        cor(d_innpendl1k, pct_syssel, use = "complete.obs", method = "spearman")
      else NA_real_,
...
```{r}
summary(kor_kommune$kor_bef_syssel)
summary(kor_kommune_spearman$kor_bef_syssel)
```

```{r}
kor_bef_syssel <- arlig_endring_med_lag |>
  group_by(knr24) |>
  summarise(
    pearson = if (sum(complete.cases(d_bef, pct_syssel)) >= 2)
      cor(d_bef, pct_syssel, use = "complete.obs") else NA_real_,
    spearman = if (sum(complete.cases(d_bef, pct_syssel)) >= 2)
      cor(d_bef, pct_syssel, method = "spearman", use = "complete.obs") else NA_real_,
    .groups = "drop"
  )
```

```{r}
summary(kor_bef_syssel)
```

```{r}
kor_ledighet_syssel_lag <- arlig_endring_med_lag |>
  group_by(knr24) |>
  summarise(
    pearson = if (sum(complete.cases(d_andel_ledighet, pct_syssel_lag1)) >= 2)
      cor(d_andel_ledighet, pct_syssel_lag1, use = "complete.obs") else NA_real_,
    spearman = if (sum(complete.cases(d_andel_ledighet, pct_syssel_lag1)) >= 2)
      cor(d_andel_ledighet, pct_syssel_lag1, method = "spearman", use = "complete.obs") else NA_real_,
    .groups = "drop"
  )
```

```{r}
summary(kor_ledighet_syssel_lag)
```

# Scatterplots og visulaiering

## Syssesetting ↔ endring i folketall

```{r}
ggplot(arlig_endring,
       aes(x = d_bef, y = pct_syssel)) +
  geom_point(alpha = 0.25, size = 1) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "Årlig endring i folketall og sysselsetting",
    x = "Endring i folketall",
    y = "Sysselsetting (%)"
  ) +
  theme_minimal() +
  coord_cartesian(xlim = c(-2000, 5000), ylim = c(-20, 25))
...
## Sentralitetsindeks ↔ boligpris

```{r}
master %>%
  filter(år %in% c(2014, 2024)) %>%
  ggplot(aes(x = index, y = KvPris_Blokkleiligheter, color = factor(år))) +
  geom_point(alpha = 0.5) +
  geom_smooth(se = FALSE) +
  labs(
    title = "Sentralitetsindeks og boligpris",
    x = "Sentralitetsindeks",
    y = "Kvadratmeter pris bolig",
    color = "År"
  ) +
  theme_minimal() +
...
## Sentralitetsindeks ↔ innpendling

```{r}
master %>%
  filter(år %in% c(2014, 2024)) %>%
  ggplot(aes(x = index, y = innPendl1k, color = factor(år))) +
  geom_point(alpha = 0.5) +
  geom_smooth(se = FALSE) +
  labs(
    title = "Sentralitetsindeks og innpendling per 1000 innbyggere",
    x = "Sentralitetsindeks",
    y = "Innpendling per 1000 innbyggere",
    color = "År"
  ) +
  theme_minimal()
```
...
## Arbeidsledighet ↔ utflytting

```{r}
master %>%
  filter(år %in% c(2014, 2024)) %>%
  ggplot(aes(x = andel_SSB.x, y = utflytting1k_tot, color = factor(år))) +
  geom_point(alpha = 0.5) +
  geom_smooth(se = FALSE) +
  labs(
    title = "Arbeidsledighet og utflytting",
    x = "Arbeidsledighet",
    y = "Utflytting",
    color = "År"
  ) +
...
```{r}
save(regresjon, file = "reg_data.rdata")
```
...
Bruke andel sysselsatte istedenfor absolutte verdier slik at man får sett det i forhold til hverandre.
...
# Bygg et "df_6"-datasett (kommune-år) fra master
df_6 <- master %>%
  transmute(
    knr24 = as.character(knr24),
    år    = as.integer(as.character(år)),

    # Folketall (BYTT om nødvendig til beftot.y)
    beft = to_num(beftot),

    # X-variabler (som dere bruker)
    Ledighet_pct      = to_num(andel_NAV),
    Hoy_utdanning_pct = to_num(UH_pct),
  ...
  mutate(
    Arbeidsplasser_1k = 1000 * Syss / beft
  ) %>%
  filter(
    is.finite(beft),
    is.finite(Ledighet_pct),
...
```{r}
# Hvilke X-er vi skal ta periodegjennomsnitt av
x_cols <- c("Ledighet_pct","Hoy_utdanning_pct","Arbeidsplasser_1k",
            "Nettoflytting_1k","Nettopendling_1k","ln_Boligpris")


# Finn start- og sluttår per kommune
df_6_period <- df_6 %>%
  group_by(knr24) %>%
  summarise(
    start_år = min(år, na.rm = TRUE),
    end_år   = max(år, na.rm = TRUE),
    .groups = "drop"
  )


# Folketall i startår og sluttår
df_6_start <- df_6 %>%
  inner_join(df_6_period, by = "knr24") %>%
  filter(år == start_år) %>%
  transmute(knr24, start_år, folketall_start = beft)

df_6_end <- df_6 %>%
  inner_join(df_6_period, by = "knr24") %>%
  filter(år == end_år) %>%
  transmute(knr24, end_år, folketall_end = beft)

# Periodegjennomsnitt av X per kommune (over hele perioden)
df_6_X_mean <- df_6 %>%
  inner_join(df_6_period, by = "knr24") %>%
  filter(år >= start_år, år <= end_år) %>%
  group_by(knr24, start_år, end_år) %>%
  summarise(
    across(all_of(x_cols), ~ mean(.x, na.rm = TRUE)),
    .groups = "drop"
  )

#Lag tverrsnittsdatasett + folketallsvekst over perioden
df_6_csB <- df_6_X_mean %>%
  inner_join(df_6_start, by = c("knr24","start_år")) %>%
  inner_join(df_6_end,   by = c("knr24","end_år")) %>%
  mutate(
    folketallsvekst = 100 * (folketall_end - folketall_start) / folketall_start
  ) %>%
  filter(is.finite(folketallsvekst)) %>%
  filter(complete.cases(across(all_of(c("folketallsvekst", x_cols)))))
```

```{r}
# Modell B: periode-gjennomsnitt-modell (OLS)
m_6B <- lm(
  folketallsvekst ~ Ledighet_pct + Hoy_utdanning_pct + Arbeidsplasser_1k +
    Nettoflytting_1k + Nettopendling_1k + ln_Boligpris,
  data = df_6_csB
)
```
...
B_vcov <- vcovHC(m_6B, type = "HC1")

tab_B <- coeftest(m_6B, vcov = B_vcov)
```
...
Modell m_6B estimerer sammenhengen mellom samlet prosentvis folketallsvekst over perioden og gjennomsnittlige strukturelle kjennetegn ved kommunene i samme periode.
...
```{r}
# Robuste vcov (tilpass cluster-variabelnavn!)
cs_vcov   <- vcovCL(mod_oppg6,      cluster = ~ kommune)  # hvis mod_oppg6 bruker "kommune"
B_vcov    <- vcovHC(m_6B,           type = "HC1")
mean_vcov <- vcovHC(mod_oppg6_mean, type = "HC1")

modelsummary(
  list(
    "Tverrsnitt (X=2010)"              = mod_oppg6,
    "Tverrsnitt (X=snitt 2000-2024)"   = mod_oppg6_mean,
    "Periode-gjennomsnitt (Modell B)"  = m_6B
  ),
  vcov = list(cs_vcov, B_vcov, mean_vcov),
  statistic = "({std.error})",
  stars = TRUE,
  gof_omit = ".*"   # behold som du hadde (skjuler GOF)
)
```

De tre modellene analyserer sammenhengen mellom strukturelle kommuneegenskaper og folketallsvekst, men benytter ulike måter å måle forklaringsvariablene på.
...
```{r}
#Sysselsetting i startår og sluttår + vekst (%)
df_7_syss <- df_6 %>%
  inner_join(df_6_period, by = "knr24") %>%
  group_by(knr24, start_år, end_år) %>%
  summarise(
    syss_start = first(Syss[år == start_år]),
    syss_end   = first(Syss[år == end_år]),
    .groups = "drop"
  ) %>%
  mutate(
    syssvekst_pct = 100 * (syss_end - syss_start) / syss_start
  ) %>%
  filter(is.finite(syssvekst_pct))

# Bygg "df_7_csB": X-gjennomsnitt + folketallsvekst + syssvekst + Sentralitet/Landsdel
df_7_csB <- df_6_csB %>%  # har: knr24, start_år, end_år, X-mean, folketallsvekst
  inner_join(
    df_7_syss %>% select(knr24, start_år, end_år, syssvekst_pct),
    by = c("knr24","start_år","end_år")
  ) %>%
  # Hent Sentralitet og Landsdel (samme definisjon som i oppg 6 tverrsnitt)
  left_join(
    x_mean %>% select(kommune, Sentralitet, Landsdel),
    by = c("knr24" = "kommune")
  ) %>%
  filter(
    is.finite(syssvekst_pct),
    is.finite(folketallsvekst),
    is.finite(Sentralitet),
    !is.na(Landsdel)
  ) %>%
  filter(complete.cases(across(all_of(c("syssvekst_pct","folketallsvekst", x_cols)))))


# (valgfritt) Antall kommuner
df_7_csB %>%
  summarise(Antall_kommuner = n()) %>%
  print()

# Oppg 7-modell (periodegjennomsnitt): sysselsettingsvekst forklart av folketallsvekst + X
m_7B <- lm(
  syssvekst_pct ~ folketallsvekst +
    Ledighet_pct + Hoy_utdanning_pct + Arbeidsplasser_1k +
    Nettoflytting_1k + Nettopendling_1k + ln_Boligpris +
...
# 4) Robuste standardfeil + tabell
vcov_7B <- vcovHC(m_7B, type = "HC1")
tab_7B  <- coeftest(m_7B, vcov = vcov_7B)

summary(m_7B)
tab_7B

save(m_7B, file = "m_7B.RData")
```

Denne modellen analyserer hvilke faktorer som henger sammen med prosentvis sysselsettingsvekst i kommunene over perioden. I denne spesifikasjonen kontrollerer vi både for strukturelle forhold som arbeidsmarked, boligpriser, sentralitet og landsdel, samt for folketallsvekst.

Den klart sterkeste sammenhengen finner vi mellom folketallsvekst og sysselsettingsvekst. Koeffisienten på 0,92 innebærer at én prosentpoeng høyere folketallsvekst er assosiert med omtrent 0,92 prosentpoeng høyere sysselsettingsvekst. Effekten er tydelig og svært statistisk signifikant. Dette er også det vi forventer teoretisk. Når befolkningen øker, øker både tilbudet av arbeidskraft og etterspørselen etter varer og tjenester. Flere innbyggere skaper behov for handel, bygg, helse og offentlige tjenester, som igjen bidrar til flere arbeidsplasser.

Nettoflytting har en positiv og svakt signifikant effekt (på 10 %-nivå). Koeffisienten på 0,18 betyr at én ekstra netto innflytter per 1000 innbyggere er forbundet med rundt 0,18 prosentpoeng høyere sysselsettingsvekst. Dette kan henge sammen med at innflyttere ofte er i yrkesaktiv alder og dermed både styrker arbeidsstyrken og øker lokal etterspørsel.

Boligpris, målt i logaritmisk form, har også en positiv og marginalt signifikant sammenheng. En økning på én prosent i boligprisen er assosiert med om lag 0,07 prosentpoeng høyere sysselsettingsvekst. Dette kan tolkes som at kommuner som fremstår som attraktive og har høy etterspørsel i boligmarkedet, også opplever noe sterkere utvikling i arbeidsmarkedet.

Sentralitet har en negativ og signifikant effekt på 5 %-nivå. Koeffisienten på -0,02 innebærer at mer sentrale kommuner, gitt lik folketallsvekst og øvrige forhold, har noe lavere sysselsettingsvekst. En mulig forklaring kan være at mye av vekstdynamikken i sentrale kommuner allerede fanges opp gjennom folketallsveksten. Når vi kontrollerer for befolkningsvekst, kan det derfor se ut som om mindre sentrale kommuner i enkelte tilfeller har relativt sterk sysselsettingsvekst.

De øvrige variablene – arbeidsledighet, andel med høyere utdanning, arbeidsplasser per innbygger og nettopendling – er ikke statistisk signifikante i denne modellen. Det kan tyde på at deres betydning for sysselsettingsvekst i stor grad virker indirekte gjennom befolkningsutviklingen. Landsdelsdummyene er heller ikke signifikante, noe som indikerer at regionale forskjeller i hovedsak fanges opp av de andre variablene i modellen.

Modellen har en justert R² på rundt 0,57, noe som betyr at omtrent 57 prosent av variasjonen i sysselsettingsvekst mellom kommunene kan forklares av variablene vi har inkludert. Samlet sett peker resultatene tydelig på at sysselsettingsvekst i stor grad følger befolkningsveksten, og at demografisk utvikling fremstår som den viktigste drivkraften bak kommunal sysselsettingsutvikling i perioden.
